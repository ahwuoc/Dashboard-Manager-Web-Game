...
const GridSpinWheel: React.FC = () => {
  // originalSegmentsData này giờ chỉ dùng để client biết các phần thưởng hiển thị là gì
  // nhưng việc chọn winner phải do server quyết định
  ...
  // useEffect để load lịch sử từ server khi component mount
  useEffect(() => {
    async function loadHistory() {
      try {
        const history = await getSpinHistoryAction(); // Call Server Action to get history
  ...
  const startSpinAnimation = useCallback(
    (winningItem: RewardItem) => {
      ...
      const spinTick = () => {
        ...
        ); // Loop a few times to ensure it finds a non-spin button
        ...
        if (progress < 1) {
          ...
          // Animation finished, ensure it lands on the winning item from server
          clearInterval(localIntervalId);
    ...
    [rewards], // rewards is a dependency as we iterate over it
  );

  const revealAllRewards = useCallback(
    (winningItem: RewardItem) => {
      // Small delay to let the final activeCell highlight sink in
      setTimeout(() => {
        setActiveCellIndex(null); // Clear active highlight after the final moment

        // Reveal all non-spin items. The winning item is already known.
  ...
  const handleSpin = async () => {
    ...
    currentIntervalSpeedRef.current = 50; // Reset speed for new spin
    setActiveCellIndex(null); // Clear previous active cell

    // Reset all revealed items before a new spin
    setRewards((prevRewards) =>
      prevRewards.map((item) => ({ ...item, revealed: false })),
    );

    try {
      const result = await spinWheelAction(); // Call Server Action

      if (result.success && result.winner) {
        // Start client-side animation with the winner from the server
        startSpinAnimation(result.winner);
      } else {
        // Handle error from server
        setErrorMessage(result.error || "Có lỗi xảy ra khi quay thưởng.");
        setIsSpinning(false); // Stop spinning on client if server error
      }
    } catch (error) {
      ...
      setIsSpinning(false); // Stop spinning on client if client-side error
    }
  };
  ...
  const handleClearHistory = async () => {
    // In a real app, you'd also call a Server Action to clear history on DB
                ...
                ${
                  item.value === "SpinButton"
                    ...
                    : item.revealed
                      ...
                      : `border-blue-700 cursor-pointer bg-gray-800/70 shadow-md`
                }
                ${
